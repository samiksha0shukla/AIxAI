...
from aixai.multi_agent import agentic_flow
...
st.set_page_config(
    page_title="AIxAI - Agent Builder",
    page_icon="🤖",
    layout="wide",
)

# Set custom theme colors to match AIxAI logo (White and Blue)
# Primary color (White) and secondary color (Blue)
st.markdown("""
    <style>
    :root {
        --primary-color: #FFFFFF;  /* White */
        --secondary-color: #000042; /* Blue */
        --text-color: #262730;
    }
...
# Function to reload the AIxAI_graph module
def reload_AIxAI_graph():
    """Reload the multi_agent module to apply new environment variables"""
    try:
        # First reload pydantic_ai_coder
        import AIxAI.pydantic_ai_coder
        importlib.reload(AIxAI.pydantic_ai_coder)
        
        # Then reload AIxAI_graph which imports pydantic_ai_coder
        import AIxAI.multi_agent
        importlib.reload(AIxAI.multi_agent)
        
        st.success("Successfully reloaded AIxAI modules with new environment variables!")
        return True
    except Exception as e:
        st.error(f"Error reloading AIxAI modules: {str(e)}")
        return False
...
def generate_mcp_config(ide_type):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run --rm -p 8100:8100 aixai:latest python mcp_server.py"
    elif ide_type == "Cline":
...
def mcp_tab():
    ...
    if st.session_state.selected_ide:
        ...
        with config_tab2:
            ...
            st.markdown("- Must be running AIxAI not within a container")           
...
async def chat_tab():
    """Display the chat interface for talking to AIxAI"""
...
def intro_tab():
    """Display the introduction and setup guide for AIxAI"""
    # Display the banner image
    st.image("public/ChatGPTImage.png", width=600)
    
    # Welcome message
    st.markdown("""
    # Welcome to AIxAI!
    
    AIxAI is an AI meta-agent designed to autonomously build, refine, and optimize other AI agents.
    
    It serves both as a practical tool for developers and as an educational framework demonstrating the evolution of agentic systems.
    AIxAI is developed in iterations, starting with a simple Pydantic AI agent that can build other Pydantic AI agents,
    all the way to a full agentic workflow using LangGraph that can build other AI agents with any framework.
    
    Through its iterative development, AIxAI showcases the power of planning, feedback loops, and domain-specific knowledge in creating robust AI agents.
    """)
    
    # Setup guide with expandable sections
    st.markdown("## Setup Guide")
    st.markdown("Follow these concise steps to get AIxAI up and running (IMPORTANT: come back here after each step):")
    ...
    with st.expander("Step 1: Environment Configuration", expanded=True):
        ...
        These settings determine how AIxAI connects to external services and which models it uses.
        """)
    ...
    with st.expander("Step 2: Database Setup", expanded=False):
        ...
        AIxAI uses Supabase for vector storage and retrieval:
    ...
    with st.expander("Step 5: MCP Configuration (Optional)", expanded=False):
        ...
        This enables you to use AIxAI directly from your AI-powered IDE.
        """)
    ...
    # Step 6: Using AIxAI
    with st.expander("Step 6: Using AIxAI", expanded=False):
        st.markdown("""
        ### Using AIxAI
        ...
        3. AIxAI will plan and generate the necessary code
        
        You can also use AIxAI directly from your AI IDE if you've configured MCP.
        """)
    ...
    - [GitHub Repository](https://github.com/samiksha0shukla/AIxAI)
    """)
            ...
            with col1:
                ...
                if st.button("Crawl Pydantic AI Docs", key="crawl_pydantic") and not (st.session_state.crawl_tracker and st.session_state.crawl_tracker.is_running):
                    try:
                        ...
                        from AIxAI.crawl_pydantic_ai_docs import start_crawl_with_requests
                ...
                if st.button("Clear Pydantic AI Docs", key="clear_pydantic"):
                    with st.spinner("Clearing existing Pydantic AI docs..."):
                        try:
                            ...
                            from AIxAI.crawl_pydantic_ai_docs import clear_existing_records
...
def database_tab():
    ...
    st.write("Set up and manage your Supabase database tables for AIxAI.")
...
def agent_service_tab():
    ...
    st.write("Start, restart, and monitor the AIxAI agent service for MCP.")
    ...
    with st.container():
        ...
        st.text_area(
            label="Realtime Logs from AIxAI Service",
            value=output_text,
...
def environment_tab():
    ...
    st.write("- Configure your environment variables for AIxAI. These settings will be saved and used for future sessions.")
...
async def main():
    ...
    with st.sidebar:
        st.image("public/ChatGPTImage.png", width=1000)
